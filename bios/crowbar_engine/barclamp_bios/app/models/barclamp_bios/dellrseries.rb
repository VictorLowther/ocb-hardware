...
class BarclampBios::Dellrseries < BarclampBios::Driver
  ...
  def fix_boot_order
    # To fix the boot order, we must get the boot order.
    bootitems = @client.enumerate("http://schemas.dell.com/wbem/wscim/1/cim-schema/2/DCIM_BootSourceSetting")
    # For now, just blanket enable all the boot sources.
    instances = []
    bootitems.Items.each do |item|
      instances << item.InstanceID.text
    end
    # NOT FINISHED YET!
  ...
  def factoy_reset!
    @client.invoke("http://schemas.dmtf.org/wbem/wscim/1/cim-schema/2/root/dcim/DCIM_LCService",
                   "LCWipe")
  end
