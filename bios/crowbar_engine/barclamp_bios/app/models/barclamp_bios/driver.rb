...
module BarclampBios
  ...
  class Driver
    ...
    # Fix up the boot order on the system so that it tries to netboot first,
    # and boot off the local hard drives second.
    def fix_boot_order
      raise "Must be implemented by subclass!"
      false
    end
    ...
    # Please not that this does not ensure that the node will PXE boot
    # by default.
    def factory_reset!
      raise "Must be implemented by subclass!"
      false
